...
fn map_scan_result(result: cl_error_t, virname: *const std::os::raw::c_char) -> Result<ScanResult, ClamError> {
    match result {
...
impl Engine {
    ...
    pub fn scan_file(&self, path: &str, settings: &mut ScanSettings) -> Result<ScanResult, ClamError> {
        ...
        unsafe {
            let mut virname: *const std::os::raw::c_char = ptr::null();
...
fn get_field_type(field: cl_engine_field) -> EngineValueType {
    match field {
        ...
        cl_engine_field::CL_ENGINE_TMPDIR_RECURSION => EngineValueType::String,
